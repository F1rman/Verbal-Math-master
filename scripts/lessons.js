  ...
  $rootScope.lesson = [{}, {
   ...
   },
    {
    ...
    },
     {
      ...
      "title": ["При зіткненні зі складним множенням", "When confronted with a difficult multiplication", "Когда сталкиваешься со сложным умножением", ],
      "content": [
        "При зіткненні зі складним множенням можна зробити спрощення. Для 89x6 замість цього вирахуйте 90x6, а потім віднімайте додаткові 6, так що 540-6 = 534.",
       "When confronted with a difficult multiplication You can do simplification. For 89x6, calculate 90x6 instead, and then subtract that additional 6, so 540-6=534.",
        "Когда сталкиваешься со сложным умножением . Для 89x6 вместо этого вычислите 90x6, а затем вычтите эти дополнительные 6, так что 540-6 = 534.", ]
    },
     {
  ...
  $rootScope.less_not_buyed = [];
  $rootScope.checkBuyedLessons = () => {
    for (var i = 4; i < $rootScope.lesson.length; i++) {
      console.log($rootScope.lesson[i]);
      if (Number(storage.getItem('lessons[' + i + ']')) == 0) {
        console.log('ne kupleno ' + i);
        $rootScope.less_not_buyed[i] = true
      } else {
        $rootScope.less_not_buyed[i] = false
