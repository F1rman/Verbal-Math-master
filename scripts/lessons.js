  app.run(function($rootScope) {
    $rootScope.lesson = [{}, {
        ...
        "content": [
          "При виконанні усної математики краще робити це рухаючись зліва направо. Переключати порядок так, що ви починаєте з найбільших значень, це робить його трохи більш інтуїтивним і легше зрозуміти. Тому при додаванні 58 до 26 починають з першого стовпця і обчислюють 50 + 20 = 70, потім 8 + 6 = 14, які додають разом 84.",
      ...
      },
      {
        ...
        "content": [
          "Віднімати в розумі двозначні числа дуже просто, якщо знаємо прийом. Описані нижче способи не відрізняються від обчислення стовпчиком, зате набагато зрозуміліше. 80 - 67 = 80 - 60 - 7 = 20 - 7 = 13 (відділення розряду одиниць при відніманні) Комбінації різних способів 78 - 49 79 - 50 (додати до чисел одиниці) 70 - 50 + 9 = 20 + 9 = 29 (відділення розряду одиниць)",
      ...
      {
        "title": [
          "Ділення / множення на 25",
          "Divide / multiply by 25",
    ...
    $rootScope.lvl_skiped = [];
    $rootScope.checklvl_skiped = () => {
      for (var i = 0; i < $rootScope.levels.length; i++) {
        if (Number(storage.getItem('level[' + i + ']')) == 0) {
          $rootScope.lvl_skiped[i] = true
        } else {
          console.log('Skiped ' + i);
          $rootScope.lvl_skiped[i] = false
        }
      }
    }
    $rootScope.checklvl_skiped();
